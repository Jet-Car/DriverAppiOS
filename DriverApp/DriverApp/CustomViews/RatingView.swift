...
//        }
        ...
        if(fullStarColor != nil){
            isAppThemeBackground = false
            isAppThemeColor = false
            
            halfImage = halfImage?.tint(with: fullStarColor)
            onImage = onImage?.tint(with: fullStarColor)
            offImage = offImage?.tint(with: emptyStarColor != nil ? emptyStarColor : UIColor.black.withAlphaComponent(0.7))
            
            
            var halfImageOn = UIImage(named: "ic_half_rating_true", in: bundle, compatibleWith: self.traitCollection)
            halfImageOn = imageResize(halfImageOn!,sizeChange: size).tint(with: fullStarColor)
            var halfImageOff = UIImage(named: "ic_half_rating_false", in: bundle, compatibleWith: self.traitCollection)
            halfImageOff = imageResize(halfImageOff!,sizeChange: size).tint(with: emptyStarColor != nil ? emptyStarColor :  UIColor.black.withAlphaComponent(0.7))
        ...
        }else if(isAppThemeBackground == true){
            halfImage = halfImage?.tint(with: UIColor.UCAColor.AppThemeTxtColor.darker(by: 20)!)
...
//    }
    ...
    func updateRating() {
        ...
        // Set every full star
        var i = 1
        let fullRatngCount = editable ? Int(rating) : (rating.truncatingRemainder(dividingBy: 0.5) > 0 ? Int(Double(rating).rounded()) : Int(rating))
