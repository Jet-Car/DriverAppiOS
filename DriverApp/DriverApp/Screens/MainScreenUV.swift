...
//            self.contentView.addSubview(self.generalFunc.loadView(nibName: "UFXMainScreenDesign", uv: self, contentView: contentView))
        ...
        //        Utils.createRoundedView(view: taxiHailImgView, borderColor: Color.clear, borderWidth: 0)
...
//        if(userProfileJson.get("eEmailVerified").uppercased() != "YES" || userProfileJson.get("ePhoneVerified").uppercased() != "YES" ){
    ...
    func setWorkLocationArea(isHide:Bool){
        if(workLocAreaHeight != nil){
            if( isHide == false){
	    // self.userProfileJson.get("APP_TYPE").uppercased() == Utils.cabGeneralType_Ride_Delivery_UberX.uppercased()
                //                self.workLocArea.backgroundColor = UIColor.UCAColor.AppThemeColor_1
    ...
    func openAvailRadius(){
        
        let manageWorkLocationUV = GeneralFunctions.instantiateViewController(pageName: "ManageWorkLocationUV") as! ManageWorkLocationUV
        manageWorkLocationUV.currentLocation = self.currentLocation
        self.pushToNavController(uv: manageWorkLocationUV)

//        let openEnterRadius = OpenEnterUFXAvailRadiusView(uv: self, containerView: self.view, userProfileJson: self.userProfileJson)
//        openEnterRadius.setViewHandler { (isRadiusChanged, radius) in
//            if(isRadiusChanged == true){
//
//                let userProfileJson = (GeneralFunctions.getValue(key: Utils.USER_PROFILE_DICT_KEY) as! String).getJsonDataDict().getObj(Utils.message_str)
//                self.userProfileJson = userProfileJson
//                self.availabilityRadiusLbl.text = "\(self.generalFunc.getLanguageLabel(origValue: "Within", key: "LBL_WITHIN")) \(radius) \(self.generalFunc.getLanguageLabel(origValue: "", key: userProfileJson.get("eUnit") == "KMs" ? "LBL_KM_DISTANCE_TXT" : "LBL_MILE_DISTANCE_TXT")) \(self.generalFunc.getLanguageLabel(origValue: "Work Radius", key: "LBL_RADIUS"))"
//
//
//                self.setUFXRadiusLblFrame()
//            }
//        }
//        openEnterRadius.show()
    }
...
//            self.gMapView.camera.zoom
    ...
    func myLableTapped(sender: MyLabel) {
        if(sender == self.changeCarLbl){
            ...
            addVehiclesUv.eType = self.userProfileJson.get("APP_TYPE")
//            (self.navigationDrawerController?.rootViewController as! UINavigationController).pushViewController(addVehiclesUv, animated: true)
    ...
    func openManageVehiclesScreen(){
        if (self.userProfileJson.get("APP_TYPE") == Utils.cabGeneralType_Ride_Delivery_UberX  || self.userProfileJson.get("APP_TYPE") == Utils.cabGeneralType_Ride_Deliver){
            ...
            appTypeSelectionUv.isFromMainUV = true
            ...
            manageVehiclesUV.eType = self.userProfileJson.get("APP_TYPE")
...
//        if userProfileJson.get("APP_TYPE") != Utils.cabGeneralType_Ride_Delivery_UberX
//        {
//            if(userProfileJson.get("ENABLE_HAIL_RIDES").uppercased() != "YES"){
//                taxiHailImgView.isHidden = true
//            }else{
//                taxiHailImgView.isHidden = false
//            }
//        }
        ...
        exeWebServerUrl.executePostProcess(completionHandler: { (response) -> Void in
            ...
            if(response != ""){
                ...
                if(dataDict.get("Action") == "1"){
                    ...
                    if(self.userProfileJson.get("APP_TYPE").uppercased() == Utils.cabGeneralType_Ride_Deliver.uppercased() || self.userProfileJson.get("APP_TYPE").uppercased() == Utils.cabGeneralType_Ride_Delivery_UberX.uppercased()){
                        heightOfRow = 60
                    }
