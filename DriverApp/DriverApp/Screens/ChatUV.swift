...
//        inputToolbar.contentView.textView.inputAccessoryView = UIView()
...
//        rightButton.setImage(UIImage(named: "ic_send_msg_arrow"), for: .normal)
        ...
        inputToolbar.contentView.rightBarButtonItemWidth = 55
        ...
        query.observe(.childAdded, with: { (snapshot) -> Void in
            if(snapshot != nil && snapshot.value != nil){
                ...
                let senderID = dataDict.get("eUserType") == Utils.appUserType ? dataDict.get("driverId") : dataDict.get("passengerId")
        ...
        receiverImgView.sd_setImage(with: URL(string: CommonUtils.passenger_image_url + receiverId + "/" + pPicName), placeholderImage: UIImage(named: "ic_no_pic_user"),options: SDWebImageOptions(rawValue: 0), completed: { (image, error, cacheType, imageURL) in
    ...
    override func didPressSend(_ button: UIButton!, withMessageText text: String!, senderId: String!, senderDisplayName: String!, date: Date!)
    {
        ...
        let msgDict = ["eUserType": Utils.appUserType, "Text": text, "iTripId": assignedtripId, "passengerImageName": pPicName, "driverImageName": userProfileJson.get("vImage"), "passengerId": receiverId, "driverId": GeneralFunctions.getMemberd()]
