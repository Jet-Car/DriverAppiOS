...
//                self.generalFunc.setError(uv: self)
  ...
  /*In Zet car App :-
   - Update estimate fare for pooling in passeger app.
   - Add pooling functiomality in driver app.
   - Work on handling pooling passengers according to there locations.
            ...
            if(response != ""){
                ...
                if(dataDict.get("Action") == "1"){
                    ...
                    if(self.passengerJsonDetail_dict!.get("REQUEST_TYPE") == Utils.cabGeneralType_UberX){
                        ...
                        let refTextHeight = self.jobRefVLbl.text!.height(withConstrainedWidth: Application.screenSize.width - 30, font: UIFont(name: "Roboto-Light", size: 20)!) - 24
...
//                        if(self.passengerJsonDetail_dict!.get("eFareType") != "Regular"){
                            ...
                            let addressHeight = self.destVLbl.text!.height(withConstrainedWidth: Application.screenSize.width - 30, font: UIFont(name: "Roboto-Light", size: 20)!) - 24
...
//                        }
                        ...
                        if(msg.get("VehicleTypeName") != ""){
                            ...
                            let vTypeHeight = requestTxt.height(withConstrainedWidth: Application.screenSize.width - 30, font: UIFont(name: "Roboto-Light", size: 20)!) - 24
                            self.requestTypeLbl.text = requestTxt
                            self.requestTypeLbl.fitText()
                            self.PAGE_HEIGHT = self.PAGE_HEIGHT + vTypeHeight
                        }
                        
                        let pickUpAddressHeight = self.pickUpVLbl.text!.height(withConstrainedWidth: Application.screenSize.width - 30, font: UIFont(name: "Roboto-Light", size: 20)!) - 24
                        self.pickUpVLbl.fitText()
                            
                        self.pickUpAddViewHeight.constant = 86 + pickUpAddressHeight
                        
                        
                        let destAddHeight = self.destVLbl.text!.height(withConstrainedWidth: Application.screenSize.width - 30, font: UIFont(name: "Roboto-Light", size: 20)!) - 24
                        self.destVLbl.fitText()
