...
//                        (tripItemView.subviews[0].subviews[0] as! MyLabel).text = Configurations.convertNumToAppLocal(numStr: dataTemp.get("TripTime"))
                        ...
                        (tripItemView.subviews[0].subviews[0] as! MyLabel).text = Utils.convertDateFormateInAppLocal(date: Utils.convertDateGregorianToAppLocale(date: dataTemp.get("tTripRequestDateOrig"), dateFormate: "yyyy-MM-dd HH:mm:ss"), toDateFormate: Utils.dateFormateTimeOnly)
                         ...
                         let eRental = dataTemp.get("eRental")
                        
//                        if(self.APP_TYPE.uppercased() == "RIDE-DELIVERY" || self.APP_TYPE.uppercased() == "UBERX"){
                            if(dataTemp.get("eHailTrip").uppercased() == "YES"){
                                let hailLbl = self.generalFunc.getLanguageLabel(origValue: "Hail", key: "LBL_HAIL")
                                if eRental.uppercased() == "YES"{
                                    (tripItemView.subviews[0].subviews[1] as! MyLabel).text = hailLbl + "-" + self.generalFunc.getLanguageLabel(origValue: "", key : "LBL_RENTAL_CATEGORY_TXT")
                                }else{
                                    (tripItemView.subviews[0].subviews[1] as! MyLabel).text = hailLbl
                                }
                            }else{
                                if(dataTemp.get("eType") == Utils.cabGeneralType_Ride){
                                    let rideLbl = self.generalFunc.getLanguageLabel(origValue: "", key: "LBL_RIDE")
                                    if eRental.uppercased() == "YES"{
                                        (tripItemView.subviews[0].subviews[1] as! MyLabel).text = rideLbl + "-" + self.generalFunc.getLanguageLabel(origValue: "", key : "LBL_RENTAL_CATEGORY_TXT")
                                    }else{
                                        (tripItemView.subviews[0].subviews[1] as! MyLabel).text = rideLbl
                                    }
                                ...
                                }else if(dataTemp.get("eType") == Utils.cabGeneralType_UberX){
                                    (tripItemView.subviews[0].subviews[1] as! MyLabel).text = dataTemp.get("vVehicleType")
                                }
				
				//else if(dataTemp.get("eType") == Utils.cabGeneralType_UberX)
                               // {
                                 //   (tripItemView.subviews[0].subviews[1] as! MyLabel).text = self.generalFunc.getLanguageLabel(origValue: "", key: "LBL_SERVICES")
                                //}
                            }
