...
//    }
        ...
        exeWebServerUrl.executePostProcess(completionHandler: { (response) -> Void in
            ...
            if(response != ""){
                ...
                if(dataDict.get("Action") == "1"){
                    ...
                    self.loadTripFinishView(eType: dataDict.get("eType"))
    ...
    func loadTripFinishView(eType:String){
        ...
        (tripFinishView.subviews[1] as! MyLabel).text = self.generalFunc.getLanguageLabel(origValue: "", key: self.tripData!.get("eType").uppercased() == Utils.cabGeneralType_UberX.uppercased() ? "LBL_JOB_FINISHED" : (self.tripData!.get("eType").uppercased() == Utils.cabGeneralType_Deliver.uppercased() ? "LBL_DELIVERY_SUCCESS_FINISHED" : "LBL_SUCCESS_FINISHED"))
        
        (tripFinishView.subviews[2] as! MyLabel).text = self.generalFunc.getLanguageLabel(origValue: "", key: self.tripData!.get("eType").uppercased() == Utils.cabGeneralType_Deliver.uppercased() ? "LBL_DELIVERY_FINISHED_TXT" : (self.tripData!.get("eType").uppercased() == Utils.cabGeneralType_UberX.uppercased() ? "LBL_JOB_FINISHED_TXT" : "LBL_TRIP_FINISHED_TXT"))
