...
class ListOfDocumentUV: UIViewController, UITableViewDelegate, UITableViewDataSource, MyBtnClickDelegate {
    ...
    var isRedirectFromAppTypeSelView = false
...
//        self.contentView.addSubview(self.generalFunc.loadView(nibName: "ListOfDocumentScreenDesign", uv: self, contentView: contentView))
    ...
    override func closeCurrentScreen() {
        ...
        var isListTypeScreenAvail = false
...
//                break
            ...
            if(viewController.isKind(of: AppTypeSelectionUV.self)){
                isListTypeScreenAvail = true
//                break
            }
        }
        
        if(isVehicleListingAvail == false){
            if(isRedirectFromAppTypeSelView == true){
                self.navigationController?.popViewController(animated: true)
            }else{
                if(isListTypeScreenAvail && fromAddVehicle){
                    self.performSegue(withIdentifier: "unwindToAppTypeSelection", sender: self)
                }else{
...
//        })
    ...
    func getListData(){
        ...
        let parameters = ["type":"displayDocList","iMemberId": GeneralFunctions.getMemberd(), "doc_usertype": self.LIST_TYPE, "iDriverVehicleId": iDriverVehicleId, "eType": eType]
    ...
    func docImgViewTapped(sender:UITapGestureRecognizer){
        ...
        if(item.get("doc_file") != ""){
            self.present(SFSafariViewController(url: NSURL(string: item.get("vimage").addingPercentEncoding(withAllowedCharacters: .urlFragmentAllowed)!)! as URL), animated: true, completion: nil)
        }
    }
