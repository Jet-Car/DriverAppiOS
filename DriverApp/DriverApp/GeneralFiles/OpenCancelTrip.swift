...
class OpenCancelTrip: NSObject {
    ...
    func cancelTrip(){
        ...
        cancelTripView.frame.size = CGSize(width: width, height: 280)
...
//        }
        ...
        if(self.uv == nil){
            ...
            self.uv.navigationController?.view.addSubview(cancelTripView)
            
            cancelTripView.tag = Utils.ALERT_DIALOG_CONTENT_TAG
...
//        self.uv.view.addSubview(cancelTripView)
        ...
        (cancelTripView.subviews[0] as! MyLabel).text = self.generalFunc.getLanguageLabel(origValue: "", key: tripData!.get("REQUEST_TYPE") == Utils.cabGeneralType_Deliver ? "LBL_CANCEL_DELIVERY" : (tripData!.get("REQUEST_TYPE") == Utils.cabGeneralType_UberX ? "LBL_CANCEL_JOB" : "LBL_CANCEL_TRIP"))
...
//        (cancelTripView.subviews[4].subviews[0] as! MyLabel).text = self.generalFunc.getLanguageLabel(origValue: tripData!.get("REQUEST_TYPE") == Utils.cabGeneralType_Deliver ? "Cancel delivery now" : "", key: tripData!.get("REQUEST_TYPE") == Utils.cabGeneralType_Deliver ? "LBL_CANCEL_DELIVERY_NOW" : (tripData!.get("REQUEST_TYPE") == Utils.cabGeneralType_UberX ? "LBL_CANCEL_JOB_NOW" : "LBL_CANCEL_TRIP_NOW"))
//
//        (cancelTripView.subviews[4].subviews[1] as! MyLabel).text = self.generalFunc.getLanguageLabel(origValue: tripData!.get("REQUEST_TYPE") == Utils.cabGeneralType_Deliver ? "Continue delivery" : "", key: tripData!.get("REQUEST_TYPE") == Utils.cabGeneralType_Deliver ? "LBL_CONTINUE_DELIVERY" : (tripData!.get("REQUEST_TYPE") == Utils.cabGeneralType_UberX ? "LBL_CONTINUE_JOB" : "LBL_CONTINUE_TRIP_TXT"))
        
        (cancelTripView.subviews[4].subviews[0] as! MyLabel).text = self.generalFunc.getLanguageLabel(origValue: "Yes", key: "LBL_YES")
        (cancelTripView.subviews[4].subviews[1] as! MyLabel).text = self.generalFunc.getLanguageLabel(origValue: "No", key: "LBL_NO")
