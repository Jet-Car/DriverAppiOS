...
class OpenAdditionalChargesView: NSObject, MyLabelClickDelegate {
    ...
    func show(currentFare:String){
        ...
        bgView.frame = CGRect(x:0, y:0, width: self.containerView.frame.width, height: Application.screenSize.height)
        bgView.center = CGPoint(x: self.containerView.frame.width / 2, y: Application.screenSize.height / 2)

        let width = (self.containerView.frame.width > 390 ? 375 : (self.containerView.frame.width - 50))
        
        let extraHeight = self.generalFunc.getLanguageLabel(origValue: "", key: "LBL_ADDITONAL_CHARGE_HINT").height(withConstrainedWidth: width - 20, font: UIFont(name: "Roboto-Medium", size: 17)!)
        
        additionalChargesView = AdditionalChargesView(frame: CGRect(x: self.containerView.frame.width / 2, y: Application.screenSize.height / 2, width: width, height: 302 + extraHeight))
        
        additionalChargesView.center = CGPoint(x: self.containerView.frame.width / 2, y: Application.screenSize.height / 2)
...
//
//        additionalChargesView = AdditionalChargesView(frame: CGRect(x: Application.screenSize.width / 2, y: Application.screenSize.height / 2, width: width, height: 302 + extraHeight))
//        additionalChargesView.center = CGPoint(x: Application.screenSize.width / 2, y: Application.screenSize.height / 2)
        ...
        if(self.uv == nil){
            ...
            currentWindow?.addSubview(additionalChargesView)
        }else if(self.uv.navigationController != nil){
            self.uv.navigationController?.view.addSubview(bgView)
            self.uv.navigationController?.view.addSubview(additionalChargesView)
            ...
            additionalChargesView.tag = Utils.ALERT_DIALOG_CONTENT_TAG
        ...
        additionalChargesView.materialFeeTxtField.setPlaceHolder(placeHolder: Configurations.convertNumToAppLocal(numStr: "0.00"))
        additionalChargesView.miscFeeTxtField.setPlaceHolder(placeHolder: Configurations.convertNumToAppLocal(numStr: "0.00"))
        additionalChargesView.providerDiscountTxtField.setPlaceHolder(placeHolder: Configurations.convertNumToAppLocal(numStr: "0.00"))
