...
//        }
      ...
      //  ****************FOR POOLING********************
        ...
        (passengerDetailView.subviews[0] as! MyLabel).text = self.generalFunc.getLanguageLabel(origValue: "", key: (self.tripArrayForPooling[0] as! NSDictionary).get("REQUEST_TYPE") == Utils.cabGeneralType_UberX ? "LBL_USER_DETAIL" : "LBL_PASSENGER_DETAIL")
        
        (passengerDetailView.subviews[1] as! MyLabel).text = (self.tripArrayForPooling[0] as! NSDictionary).get("PName")
        (passengerDetailView.subviews[2] as! MyLabel).text = Configurations.convertNumToAppLocal(numStr: (self.tripArrayForPooling[0] as! NSDictionary).get("PRating"))
        
        (passengerDetailView.subviews[3] as! UIImageView).sd_setImage(with: URL(string: CommonUtils.passenger_image_url + (self.tripArrayForPooling[0] as! NSDictionary).get("PassengerId") + "/" + (self.tripArrayForPooling[0] as! NSDictionary).get("PPicName")), placeholderImage:UIImage(named:"ic_no_pic_user"))
        
        (passengerDetailView.subviews[5] as! RatingView).rating = GeneralFunctions.parseFloat(origValue: 0.0, data: (self.tripArrayForPooling[0] as! NSDictionary).get("PRating"))
